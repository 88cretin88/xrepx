...
create_build_scripts () {
	...
	echo 'apt-get -y build-dep wine wine-development' >> $MAINDIR/prepare_chroot.sh
	echo 'apt-get install -y software-properties-common' >> $MAINDIR/prepare_chroot.sh

#	echo 'add-apt-repository -y ppa:ubuntu-toolchain-r/test' >> $MAINDIR/prepare_chroot.sh
#	echo 'apt-get update' >> $MAINDIR/prepare_chroot.sh
#	echo 'apt-get install g++-7 -y' >> $MAINDIR/prepare_chroot.sh
#	echo 'update-alternatives --install /usr/bin/gcc gcc /usr/bin/gcc-7 60 --slave /usr/bin/g++ g++ /usr/bin/g++-7 ' >> $MAINDIR/prepare_chroot.sh
#	echo 'update-alternatives --config gcc' >> $MAINDIR/prepare_chroot.sh 

	echo 'apt-get -y install libsdl2-dev libpulse-dev libudev-dev libvulkan-dev libsane-dev libv4l-dev libkrb5-dev libgphoto2-dev liblcms2-dev libpcap-dev libcapi20-dev gcc-mingw-w64 g++-mingw-w64 ocl-icd-dev ocl-icd-opencl-dev ocl-icd-libopencl1 opencl-headers libfreetype6-dev curl fontforge fonttools python3-tk  gcc-multilib g++-multilib libfreetype6-dev xserver-xorg-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev && sleep 5' >> $MAINDIR/prepare_chroot.sh
